...
__kernel void calc_distances(
    ...
    float distances[MAX_BLOCK_COUNT_2];
        ...
        for (int i = WINDOW_SIZE_HALF % window_step_size; i < WINDOW_SIZE; i += window_step_size) {
            ...
            for (int y = 0; y < BLOCK_SIZE; y++) {
                for (int x = 0; x < BLOCK_SIZE; x++) {
                    ...
                    ref[y][x] = read_imagef(input, sampler, ref_pos).s0;
                }
            }
            ...
            for (int y = 0; y < BLOCK_SIZE; y++) {
                for (int x = 0; x < BLOCK_SIZE; x++) {
                    ...
                    const float b = read_imagef(input, sampler, pos).s0;
            ...
            for (int n = 0; n < block_count; n++) {
                ...
                for (int j = 0; j < BLOCK_SIZE; j++) {
                    for (int i = 0; i < BLOCK_SIZE; i++) {
                        ...
                        block[j][i] = read_imagef(input, sampler, pos).s0;
                    }
                }
...
#   pragma unroll
        ...
        for (int i = 0; i < SPLIT_SIZE_X; i++) {
            ...
            if (pos.x < WIDTH && pos.y < HEIGHT) {
                ...
                //float pixel_value = ACCU(i, j) / WM(i, j);
                float w = WM(i, j);
                float pixel_value = (w != 0.0f) ? (ACCU(i, j) / w) : 0.0f;
                write_imagef(output, pos, pixel_value);
            }
        }
    }
#endif
            ...
            for (int n = 0; n < block_count; n++) {
                ...
                for (int j = 0; j < BLOCK_SIZE; j++) {
                    for (int i = 0; i < BLOCK_SIZE; i++) {
                        const int2 pos = {ri - WINDOW_SIZE_HALF + similar_coords[2*(n*tot_globals_d + rgid)] + i,
                                          rj - WINDOW_SIZE_HALF + similar_coords[2*(n*tot_globals_d + rgid)+1] + j};
                        basic_block[j][i] = read_imagef(basic, sampler, pos).s0;
                        noise_block[j][i] = read_imagef(input, sampler, pos).s0;
                    }
                }
...
#   pragma unroll
        ...
        for (int i = 0; i < SPLIT_SIZE_X; i++) {
            const int2 pos = {gid.x + i, gid.y + j};
            if (pos.x < WIDTH && pos.y < HEIGHT) {
                //const float pixel_value = ACCU(i, j) / WM(i, j);
                float w = WM(i, j);
                float pixel_value = (w != 0.0f) ? (ACCU(i, j) / w) : 0.0f;
                write_imagef(output, pos, pixel_value);
            }
