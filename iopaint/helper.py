...
def norm_img(np_img):
    ...
    return to_float32(np_img)

def to_float32(img):
    if img.dtype == np.uint8:       img = img.astype(np.float32) / 255
    elif img.dtype == np.uint16:    img = img.astype(np.float32) / 65535
    elif img.dtype == np.uint32:    img = img.astype(np.float32) / 4294967295
    elif img.dtype == np.float32:   pass
    else:                           img = img.astype(np.float32) / 255

    return img

def to_uint8(img):
    if img.dtype == np.uint8:       pass
    elif img.dtype == np.uint16:    img = (img / 256).astype(np.uint8)
    elif img.dtype == np.float32:   img = (img * 255).astype(np.uint8)
    elif img.dtype == np.float64:   img = (img * 255).astype(np.uint8)
    else:                           img = (img / 256).astype(np.uint8)
...
def gen_frontend_mask(bgr_or_gray_mask):
    if len(bgr_or_gray_mask.shape) == 3 and bgr_or_gray_mask.shape[2] != 1:
        ...
        bgr_or_gray_mask = cv2.cvtColor(bgr_or_gray_mask, cv2.COLOR_RGB2GRAY)
