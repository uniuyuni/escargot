...
class Train:
    def __init__(self, args):
        ...
        if self.gpu_ids and torch.backends.mps.is_available():
            #self.device = torch.device("cuda:%d" % self.gpu_ids[0])
            self.device = torch.device("mps")
            #torch.mps.set_device(self.gpu_ids[0])
        else:
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader_test, 1):
                ...
                for n, block in enumerate(input_blocks, 0):
                    ...
                    #_in = skimage.color.rgb2ycbcr(_in)
                    #_in[:,:,1] = skimage.restoration.denoise_tv_bregman(_in[:,:,1])
                    #_in[:,:,2] = skimage.restoration.denoise_tv_bregman(_in[:,:,2])
                    #_in = skimage.color.ycbcr2rgb(_in)

                    _in = torch.from_numpy(_in.transpose((2, 0, 1)).astype(np.float32)).unsqueeze(0).to(device)
                    _out = netG(_in)
                    _out = _out*0.5+0.5
                    _out = torch.clamp(_out, 0., 1.)
                    _out = _out.to('cpu').detach().numpy().transpose(0, 2, 3, 1)[0,:,:,:].squeeze()
                    #_out = np.delete(_out, [0, 1, 2, 3, 4, 5, 6, 7, 1024+16-8, 1024+16-7, 1024+16-6, 1024+16-5, 1024+16-4, 1024+16-3, 1024+16-2, 1024+16-1], 0)
                    #_out = np.delete(_out, [0, 1, 2, 3, 4, 5, 6, 7, 1024+16-8, 1024+16-7, 1024+16-6, 1024+16-5, 1024+16-4, 1024+16-3, 1024+16-2, 1024+16-1], 1)
                    _out = np.delete(_out, [0, 1, 2, 3], 1)
                    #plt.imsave(os.path.join(dir_result_test, 'images', "0000-%04d-output.png"%n), _out, cmap=cmap)
                ...
                for j in range(label.shape[0]):
                  ...
                  #  plt.imsave(os.path.join(dir_result_test, 'images', fileset['output']), output, cmap=cmap)
                    #label = bm3d.bm3d(output, 0.5)
                    #label = skimage.filters.unsharp_mask(label)
                    #label = np.clip(label, 0, 1)
                    #plt.imsave(os.path.join(dir_result_test, 'images', fileset['label']), label, cmap=cmap)
